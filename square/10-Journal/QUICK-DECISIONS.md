...
- 맵 화면은 전체 화면 뷰로, 대화↔맵 토글 전환 방식으로 한다.
- 방 클릭 시 맵 유지 + 슬라이드 오버레이(데스크탑), 바텀시트(모바일).
- 맵은 추상 플로어플랜 — 타일 바닥 없이 테마 색면 카드 + 캣로프 아바타.
- 테마 색상은 채팅창 배경에 적용한다. 맵 카드에는 10-15% opacity로 힌트만 준다.
- 타일셋 에셋은 추상 플로어플랜으로 결정했으므로 불필요. tests/tiles 폴더는 정리 대상.
