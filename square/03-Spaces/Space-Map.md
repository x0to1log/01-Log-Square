...
> 01 Log Square의 가상 오피스는 단순한 메뉴 구조가 아니라, `오피스 도면이 네비게이션이 되는 하이브리드 플로어플랜 워크스페이스`다.
...
| [[01-Mobile]] | 대표용 빠른 확인, 승인, 메모 채널 |

## 공간 설계 원칙

- 공간마다 발언자와 기대 산출물이 달라야 한다.
- 공간 이동은 사용자의 정신 모델을 바꾸는 장치여야 한다.
- 프로젝트 중심 공간과 조직 중심 공간을 분리한다.
- 기록 공간은 늘 뒤에서 작동하지만 언제든 되짚어볼 수 있어야 한다.
- 도면은 장식이 아니라 현재 상태와 역할을 보여주는 운영 지도여야 한다.
- 픽셀 요소는 공간 정체성과 상태 표현에만 쓰고, 실제 업무 텍스트는 현대적 UI로 유지한다.
